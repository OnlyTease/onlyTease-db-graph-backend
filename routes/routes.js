...
const SubscriptionBase = require("../models/subscriptionBase")
...
router.get('/user-info-base', async (req, res) => {
  ...
  try {
    ...
    // Fetch Base-specific subscriptions and populate model details
    const subscriptions = await SubscriptionBase.find({ user: user._id })
      .populate({
        path: 'model',
        select: 'name modelId ipfsUrl' // Selecting specific fields from the Model
      })
...
router.get('/user-model-info', async (req, res) => {
  ...
  try {
    const subscription = await SubscriptionBase.findOne({ tokenId }).populate('user model');
...
// Purchase a subscription
router.post('/purchase-subscription-base', async (req, res) => {
  const { email, modelId, tokenId } = req.body;
  try {
    // Ensure both user and model exist based on email and modelId
    const userExists = await User.findOne({ email: email });
    ...
    // Create subscription using the ids from the fetched documents
    const newSubscription = new SubscriptionBase({
      user: userExists._id,
      model: modelExists._id,
      tokenId
    });
...
router.patch('/list-subscription', async (req, res) => {
  const { tokenId, listingId,price } = req.body;
  try {
    const updatedSubscription = await Subscription.findOneAndUpdate(
      { tokenId },
      { $set: { price,listingId, isListed: true } },
      { new: true }
    );
...
// List a subscription
router.patch('/list-subscription-base', async (req, res) => {
  const { tokenId, listingId,price } = req.body;
  try {
...
// Update subscription's user
router.patch('/update-subscription-base', async (req, res) => {
  const { tokenId, wallet_address } = req.body;
  try {
    const user = await User.findOne({ wallet_address });
    if (!user) {
...
// Fetch all listed subscriptions
router.get('/listed-subscriptions-base', async (req, res) => {
  try {
    const subscriptions = await SubscriptionBase.find({ isListed: true }).populate('model', 'modelId');
    res.status(200).json({
      success: true,
      message: 'Listed subscriptions retrieved successfully',
      data: subscriptions
